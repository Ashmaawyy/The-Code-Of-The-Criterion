        ...
        for line in f:
            ...
            # --- lemmas ---
                ...
                for row in _parse_copy_block(f, cols):
                    ...
                    if lid:
                        lemmas[lid] = {
                            "text": row.get('text_madani', '') or row.get('text_clean', ''),
                ...
                for row in _parse_copy_block(f, cols):
                    ...
                    if wid:
                        segments[wid].append({
                            ...
                            "grammar_desc_en": row.get('grammar_term_desc_english', ''),
                            "grammar_desc_ar": row.get('grammar_term_desc_arabic', ''),
                            "segment_type": row.get('segment_type', ''),
                        })

    elapsed = time.monotonic() - start
    logger.info("Parsed: %d roots, %d lemmas, %d words, %d segment entries in %.1fs",
                len(roots), len(lemmas), len(words), sum(len(v) for v in segments.values()),
                elapsed)
...
def main():
    ...
    parser.add_argument("--index", default=QAC_INDEX_NAME,
                        help=f"Target index name (default: {QAC_INDEX_NAME})")
    parser.add_argument("--dry-run", action="store_true",
                        help="Parse SQL but don't index to ES")
    ...
    if args.dry_run:
        logger.info("[DRY RUN] Parsed %d verses, would index to %s",
                    len(data.get("by_verse", {})), args.index)
        # Show a sample
        by_verse = data.get("by_verse", {})
        ...
        if sample_key:
            ...
            for w in sample[:3]:
                logger.info("  pos=%d text=%s root=%s pos_tags=%s",
                            w.get("position", 0), w.get("text_clean", ""),
                            w.get("root", ""), w.get("pos_tags", []))
        return

...
def display_stored_verdict(data: dict) -> None:
    ...
    for g in data.get("gate_scores", []):
        ...
        print(
            f"    {status_marker} {g.get('name', '?')}: {g.get('score', 0)}/100 — "
            f"{g.get('result', '?')}"
        )
...
def build_corrected_verdict(original: Verdict) -> Verdict:  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    """
    ...
    if choice == "y":
        friction_points = prompt_list(
            "  Enter corrected friction points (empty line to finish):"
        )
    else:
    ...
    if choice == "y":
        ...
        for g in original.gate_scores:
            ...
            new_result = (
                GateResult.SURVIVE if result_str == "survive" else GateResult.FAIL
            )
            reasoning = prompt_text(
                f"      Reasoning [{g.reasoning[:50]}...]: ", allow_empty=True
            )
            gate_scores.append(
                GateScore(
                    name=g.name,
...
class HumanReview:
    ...
    def __init__(self, store: VerdictStore):  # pylint: disable=redefined-outer-name
        self.store = store
    ...
    def browse_verdicts(self) -> None:  # pylint: disable=too-many-locals,too-many-branches,too-many-nested-blocks
        ...
        display_list = verdict_docs
        for i, hit in enumerate(display_list, 1):
            data = hit["_source"]
                ...
                if 0 <= idx < len(display_list):
                    ...
                    if action == "i":
                        ...
                        if verdict_id:
                            ...
                            if flagged:
                                print(
                                    f"  Flagged {len(flagged)} downstream verdicts "
                                    "for re-review:"
                                )
                                for fid in flagged:
                                    print(f"    - {fid}")
    ...
    def search_verdicts(self) -> None:
        ...
        for i, r in enumerate(results, 1):
            ...
            print(
                f"  {i}. [dist: {dist_str}] (score: {meta.get('total_score', '?')}) "
                f"{doc_preview}..."
            )
...
def run_review_session(verdict_store: VerdictStore) -> None:  # pylint: disable=too-many-branches,too-many-statements,too-many-nested-blocks
    """
    ...
    """
    review = HumanReview(verdict_store)
    ...
    while True:  # pylint: disable=too-many-nested-blocks
        ...
        if choice == "1":
            ...
            # Find verdicts with needs_review status via ES
            resp = verdict_store._es.search(
                index=verdict_store._index,
                body={"query": {"term": {"status": "needs_review"}}, "size": 50},
            )
            pending = [h["_source"] for h in resp["hits"]["hits"]]
            ...
            while True:
                ...
                try:
                    ...
                    if 0 <= idx < len(pending):
                        ...
                        if action == "a":
                            verdict_store.update_status(verdict_id, "approved")
                            print(f"  Verdict {verdict_id} re-approved.")
                        elif action == "r":
                            verdict_store.update_status(verdict_id, "rejected")

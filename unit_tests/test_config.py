...
class TestDefaults:
    def test_app_config_defaults(self):
        ...
        config = AppConfig()
        logger.debug("llm.provider=%s, llm.model_name=%s", config.llm.provider, config.llm.model_name)
        ...
        logger.debug("review.auto_approve_threshold=%s, review.show_reasoning_detail=%s, review.max_browse_count=%d",
...
"""
        ...
        logger.debug("Loaded: provider=%s, model=%s, temp=%s, passes=%d, collection=%s, threshold=%s",
...
"""
        ...
        config = load_config(config_path)
        logger.debug("Loaded model_name=%s, provider=%s (should be default), passes=%d (should be default)",
                      config.llm.model_name, config.llm.provider, config.engine.max_correction_passes)
...
class TestSaveConfig:
    def test_save_and_reload(self, tmp_path):
        ...
        config_path = tmp_path / "config.yaml"
        logger.info("Saving config to %s (provider=transformers, model=test_model, passes=7)", config_path)

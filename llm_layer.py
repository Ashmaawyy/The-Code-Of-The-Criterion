...
class LLMConfig:
    ...
    provider: str = "ollama"            # ollama, transformers, openai_compatible
    model_name: str = "mistral"         # model identifier
    base_url: str = "http://localhost:11434"  # API base URL (Ollama / OpenAI-compatible)
...
def create_llm_from_dict(d: dict) -> LLMProvider:
    ...
    config = LLMConfig(**{k: v for k, v in d.items() if k in LLMConfig.__dataclass_fields__})

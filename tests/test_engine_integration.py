...
class TestEngineModules:
    ...
    def test_engine_modules_consistent(self):
        ...
        from al_furqan.engine.axioms import AXIOMS, GATE_DEFINITIONS, AXIOM_HASH, AXIOM_VERSION  # pylint: disable=import-outside-toplevel
        from al_furqan.engine.models import SystemType, GateResult  # pylint: disable=import-outside-toplevel
        from al_furqan.engine.prompts import build_scan_prompt  # pylint: disable=import-outside-toplevel
        from al_furqan.engine.pipeline import EvaluationPipeline  # pylint: disable=import-outside-toplevel
    ...
    def test_engine_init_exports(self):
        ...
        from al_furqan.engine import models, axioms, prompts, pipeline  # pylint: disable=import-outside-toplevel
...
class TestBackwardCompatibility:
    ...
    def test_backward_compatibility_reasoning_engine(self):
        ...
        from al_furqan.core.reasoning_engine import ReasoningEngine  # pylint: disable=import-outside-toplevel
        from al_furqan.core.reasoning_engine import Verdict, GateScore  # pylint: disable=import-outside-toplevel
        from al_furqan.core.reasoning_engine import AXIOM_HASH  # pylint: disable=import-outside-toplevel
    ...
    def test_old_and_new_imports_are_same_objects(self):
        ...
        from al_furqan.core.reasoning_engine import Verdict as OldVerdict  # pylint: disable=import-outside-toplevel
        from al_furqan.engine.models import Verdict as NewVerdict  # pylint: disable=import-outside-toplevel

        assert OldVerdict is NewVerdict, "Old and new Verdict should be the same class"

        from al_furqan.core.reasoning_engine import AXIOM_HASH as old_hash  # pylint: disable=import-outside-toplevel
        from al_furqan.engine.axioms import AXIOM_HASH as new_hash  # pylint: disable=import-outside-toplevel

        assert old_hash == new_hash, "Axiom hash should be identical via old and new paths"
...
class TestAxiomHashConsistency:
    ...
    def test_axiom_hash_consistency(self):
        ...
        from al_furqan.engine.axioms import AXIOM_HASH as hash1  # pylint: disable=import-outside-toplevel

        # Force a fresh computation by importing the function
        from al_furqan.engine.axioms import _compute_axiom_hash  # pylint: disable=import-outside-toplevel
    ...
    def test_axiom_hash_is_sha256(self):
        ...
        from al_furqan.engine.axioms import AXIOM_HASH  # pylint: disable=import-outside-toplevel

        assert all(c in "0123456789abcdef" for c in AXIOM_HASH), "Hash should be lowercase hex"
    ...
    def _make_mock_llm(self):
        ...
        def mock_llm(prompt: str) -> str:
            ...
            if "the mirror" in prompt_lower:  # pylint: disable=no-else-return
                return json.dumps({
                    "gate_scores": [
                        {"name": "Source-Integrity", "score": 80, "result": "Survive", "reasoning": "Clear evidence"},  # pylint: disable=line-too-long
                        {"name": "Maqasid-Alignment", "score": 75, "result": "Survive", "reasoning": "Protects wealth"},  # pylint: disable=line-too-long
                        {"name": "Contextual-Sensitivity", "score": 70, "result": "Survive", "reasoning": "Context aware"},  # pylint: disable=line-too-long
                    ],
                    "origin_gate": "Survive",
                })

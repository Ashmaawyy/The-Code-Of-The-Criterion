...
from al_furqan.api.orchestrator import Orchestrator, EvaluationResult
...
class TestFullPipelineE2E:
    ...
    def test_single_source_of_truth_all_fail(self):
        ...
        response_llm = MagicMock(return_value="Single source of truth is rejected by the evaluation.")  # pylint: disable=line-too-long
        ...
        assert result.response_text == "Single source of truth is rejected by the evaluation."
    ...
    def test_pipeline_with_z3_verification(self):
        ...
        orch = Orchestrator(
            engine_pipeline=pipeline, symbolic_verifier=verifier
        )
    ...
    def test_verdict_contains_gate_details(self):
        ...
        for gs in result.verdict.gate_scores:
            assert gs.name in ["Source-Integrity", "Structural-Consistency", "Mediation-Zeroing"]
